...
Application.execute()
  ...
  │         ├─ upWebiny.reconcile() — pin independently-versioned packages (di, stdlib, wts-client) from references.json
...
| `ReferencesService`     | `service/References/index.js`     | `getReference(name): IReference \| null`, `getVersion(name): string \| null` — looks up canonical package versions from `references.json`. Reads from disk on every call.                                                                                                                                                                                                                                                           |
