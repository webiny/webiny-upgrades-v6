...
| `ReferencesService` | `../../service/References/index.js` | `getReference(name): IReference \| null`, `getVersion(name): string \| null` — looks up canonical package versions from `references.json`. `clearCache()` — discards the in-memory cache so the next call re-reads from disk. |
