...
| `WebinyConfigTool` | `../../tool/WebinyConfigTool/index.js` | Reads and mutates `webiny.config.tsx` via ts-morph AST. `read(): WebinyConfigFile` (throws if not found), `save(file): void`. `addChild(tag, opts)` is duplicate-safe: warns and skips if the element already exists; provides structural merge when a `children` callback is supplied on an existing parent. See **WebinyConfigFile API** below. |
